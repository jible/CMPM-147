...
let speed = .05; // This is for storing time!
let deltaTime = 0;
...
function draw() {
  ...
  if ( rightDown){
    ...
    if (window.p3_tileErased) {
      window.p3_tileErased(world_pos[0], world_pos[1]);
    }
  }
  ...
  if ( leftDown){
    ...
    if (window.p3_tileClicked) {
      window.p3_tileClicked(world_pos[0], world_pos[1], -1);
    }
  } else if (rightDown){
    let world_pos = screenToWorld(
      [0 - mouseX, mouseY],
